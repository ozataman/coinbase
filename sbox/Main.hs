...
fullChannel = [ChannelSubscription FullChannel [btc]]

compactChannels =
  [ ChannelSubscription Level2Channel [btc]
  , ChannelSubscription TickerChannel [btc]
  ]
...
printSocket = do
        conf <- mkConf
        subscribe (Just conf) Live (fullChannel ++ compactChannels) $ \conn -> do
            putStrLn "Connected."
            _ <- forkIO $ forever $ do
